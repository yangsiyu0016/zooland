 ...
 *
 */
@Component("productSplitCKLLCompleteHandler")
public class ProductSplitCKLLCompleteHandler implements TaskListener {
	...
	public void notify(DelegateTask delegateTask) {
		...
		/*
		 * //获取仓库id String warehouseId = split.getWarehouse().getId(); //获取产品id String
		 * productId = split.getProduct().getId(); //获取库存 Stock stock =
		 * stockService.getStock(productId, warehouseId);
		 * 
		 * //获取货位库存信息 StockDetail stockDetail =
		 * stockDetailService.getStockDetail(stock.getId(),
		 * split.getGoodsAllocation().getId());
		 * 
		 * //更新货位库存数量
		 * stockDetail.setUsableNumber(stockDetail.getUsableNumber().subtract(split.
		 * getNumber())); stockDetailService.updateStockDetail(stockDetail);
		 * 
		 * 
		 * 更新库存信息开始
		 * 
		 * //变化后可用数量 BigDecimal after_usableNumber =
		 * stock.getUsableNumber().subtract(split.getNumber()); //变化后库存总额 BigDecimal
		 * after_totalMoney =
		 * stock.getTotalMoney().subtract(split.getNumber().multiply(stock.getCostPrice(
		 * )));
		 * 
		 * //更新库存 stock.setUsableNumber(after_usableNumber);
		 * stock.setTotalMoney(after_totalMoney); stockService.updateStock(stock);
		 * 
		 * 更新库存信息结束
		 * 
		 * //添加库存变化明细 JournalAccount journalAccount = new JournalAccount();
