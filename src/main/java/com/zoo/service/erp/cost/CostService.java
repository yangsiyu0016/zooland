...
public class CostService {
	...
	public void addCostFromSell(Cost cost) {
		...
		for(CostDetail detail:cost.getDetails()) {
			...
			Stock stock = stockMapper.getStock(detail.getProductSku().getId(), detail.getWarehouse().getId());
			...
			outboundDetail.setNumber(detail.getNumber());
			BigDecimal ckPrice = stock.getCostPrice();
			outboundDetail.setPrice(ckPrice);
			BigDecimal totalMoney = detail.getNumber().multiply(ckPrice);
			...
			//总库存改变后的可用数量
			BigDecimal stock_afterUsableNumberChange = stock.getUsableNumber().subtract(detail.getNumber());
			//总库存改变后的锁定数量
			BigDecimal after_costPrice = stock.getCostPrice();
			BigDecimal after_totalMoney = stock.getTotalMoney().subtract(totalMoney);
			...
			if(stock_afterUsableNumberChange.compareTo(BigDecimal.ZERO)==0) {
				...
				after_costPrice = after_totalMoney.divide(stock_afterUsableNumberChange, 4, BigDecimal.ROUND_HALF_UP);
			}
			...
			if(stockDetail==null) throw new RuntimeException("货位库存不存在");
			
			BigDecimal stockDetail_afterUsableNumberChange = stockDetail.getUsableNumber().subtract(detail.getNumber());
			...
			journalAccount.setCkTotalMoney(totalMoney);
	...
	public void inboundFromPurchase(String costId) {
		...
		for(CostDetail detail:cost.getDetails()) {
			...
			inboundDetail.setGoodsAllocation(warehouse.getGoodsAllocations().get(0));
			inboundDetail.setOrderDetailId(detail.getDetailId());
			inboundDetail.setNumber(detail.getNumber());
			...
			inboundDetail.setTotalMoney(purchaseDetail.getPrice().multiply(detail.getNumber()));
			...
			if(stock==null) {
				...
				stock.setTotalMoney(purchaseDetail.getPrice().multiply(detail.getNumber()));
				
				stockMapper.addStock(stock);
				
				StockDetail stockDetail = new StockDetail();
				stockDetail.setId(UUID.randomUUID().toString());
				stockDetail.setGoodsAllocation(warehouse.getGoodsAllocations().get(0));
				stockDetail.setStockId(stock.getId());
				stockDetail.setUsableNumber(detail.getNumber());
				stockDetailMapper.addDetail(stockDetail);
			}else {
				//入库后结存总额
				BigDecimal afterInTotalMoney =  stock.getTotalMoney().add(purchaseDetail.getPrice().multiply(detail.getNumber()));
				//入库后结存总数
				BigDecimal afterIntotalNumber = stock.getUsableNumber().add(detail.getNumber());
				//入库后结存成本价
				...
				StockDetail stockDetail = stockDetailMapper.getDetail(stock.getId(), warehouse.getGoodsAllocations().get(0).getId());
				if(stockDetail==null) {
			...
			journalAccount.setRkTotalMoney(purchaseDetail.getPrice().multiply(detail.getNumber()));
			journalAccount.setCtime(new Date());
			journalAccount.setTotalNumber(stock.getUsableNumber().add(stock.getLockedNumber()==null?new BigDecimal("0"):stock.getLockedNumber()));
			journalAccount.setCompanyId(LoginInterceptor.getLoginUser().getCompanyId());
			journalAccountService.addJournalAccount(journalAccount);
