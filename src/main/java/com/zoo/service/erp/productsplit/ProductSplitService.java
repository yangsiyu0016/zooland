...
public class ProductSplitService {
		...
		for(InboundDetail detail:inbound.getDetails()) {
			...
			for(ProductSplitDetail splitDetail:split.getDetails() ) {
				if(detail.getProduct().getId().equals(splitDetail.getProduct().getId())) {
					BigDecimal notInNumber = splitDetail.getNotInNumber().subtract(detail.getNumber());
					splitDetail.setNotInNumber(notInNumber);
					detailService.updateNotInNumberById(notInNumber, splitDetail.getId());
					break;
				}
			}
		}
	...
	public void deleteIn(String splitId,String inboundDetailId,String type) {
		ProductSplit split = this.getProductSplitById(splitId);
		InboundDetail inboundDetail = inboundDetailService.getDetailById(inboundDetailId);
		Inbound inbound = inboundService.getInboundById(inboundDetail.getInboundId());
		if(type.equals("only")) {
			
			this.deleteInDetail(inboundDetail, inbound);
			...
			for(ProductSplitDetail splitDetail:split.getDetails()) {
				if(inboundDetail.getProduct().getId().equals(splitDetail.getProduct().getId())) {
					detailService.updateNotInNumberById(splitDetail.getNotInNumber().add(inboundDetail.getNumber()), splitDetail.getId());
					break;
				}
			}
		}else {
			for(InboundDetail detail:inbound.getDetails()) {
				this.deleteInDetail(detail, inbound);
				for(ProductSplitDetail splitDetail:split.getDetails()) {
					if(detail.getProduct().getId().equals(splitDetail.getProduct().getId())) {
						BigDecimal notInNumber = splitDetail.getNotInNumber().add(detail.getNumber());
						splitDetail.setNotInNumber(notInNumber);
						detailService.updateNotInNumberById(notInNumber, splitDetail.getId());
					}
				}
	...
	private void deleteInDetail(InboundDetail inboundDetail, Inbound inbound) {
		if(inboundDetail.getFinished()) {
			Stock stock = stockService.getStock(inboundDetail.getProduct().getId(), inbound.getWarehouse().getId());
			if(stock.getUsableNumber().subtract(inboundDetail.getNumber()).compareTo(BigDecimal.ZERO)==-1) {
				throw new ZooException(inboundDetail.getProduct().getName()+"库存不足");
			}else {
				StockDetail stockDetail = stockDetailService.getStockDetail(stock.getId(), inboundDetail.getGoodsAllocation().getId());
				if(stockDetail.getUsableNumber().subtract(inboundDetail.getNumber()).compareTo(BigDecimal.ZERO)==-1) {
					throw new ZooException(inboundDetail.getProduct().getName()+"在货位"+inboundDetail.getGoodsAllocation().getName()+"上库存不足");
					...
					BigDecimal after_usableNumber = stock.getUsableNumber().subtract(inboundDetail.getNumber());
					BigDecimal after_totalMoney = stock.getTotalMoney().add(inboundDetail.getTotalMoney());
					BigDecimal after_costPrice;
					if(after_usableNumber.compareTo(BigDecimal.ZERO)==0) {
						after_totalMoney = new BigDecimal("0");
						after_costPrice = stock.getCostPrice();
					...
					JournalAccount journalAccount = new JournalAccount();
					journalAccount.setId(UUID.randomUUID().toString());
					journalAccount.setType(JournalAccountType.SPLITRKDELETE);
					journalAccount.setOrderDetailId("");
